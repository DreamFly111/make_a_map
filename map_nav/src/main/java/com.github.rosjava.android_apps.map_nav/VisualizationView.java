...
public class VisualizationView extends org.ros.android.view.visualization.VisualizationView{
    ...
    private void startTransformListener() {
        final Subscriber<TFMessage> tfSubscriber =
                connectedNode.newSubscriber("tf", TFMessage._TYPE);
        ...
        final Subscriber<TFMessage> tfStaticSubscriber =
                connectedNode.newSubscriber("tf_static", TFMessage._TYPE);
        tfStaticSubscriber.addMessageListener(new MessageListener<TFMessage>() {
            @Override
            public void onNewMessage(TFMessage message) {
                synchronized (mutex) {
                    for (geometry_msgs.TransformStamped transform : message.getTransforms()) {
